'use strict';
/* =====================================================================
   RISO CORE — the drawing core of the neon risograph look (references/visuals.md).
   A risograph engine turned inside out: near-black paper, fluorescent inks, and plates that ADD
   light where they overlap instead of darkening. Every room, stat card, chart and title card in
   a project loads this one file; the single-file build and the renderer inline it.

   Adapted from the riso-rooms engine in IshaanKalra2103/creative-skills, MIT License,
   Copyright (c) 2026 Ishaan Kalra. The full notice is in ../NOTICE.txt and must travel with
   this file. First built for hedgefly (github.com/Tacdelgineer/hedgefly).

   What is in here: seeded randomness, noise and polyline helpers; the isometric projection;
   paper, inks and halftone screens; the pen (fills, hand-drawn lines, shapes, the room shell,
   furniture, lamps and glows); the characters; and a stencil alphabet drawn with the same pen.
   ===================================================================== */
const TAU=Math.PI*2;
...
/* ---------- paper and inks: every colour means one thing. Bind each to a project noun first. ---------- */
const PAPER=[7,7,11];                               // near-black stock, a violet cast, never #000
const INKS={
  cyan:[39,242,210],    // subject A of the comparison, and nothing else
  magenta:[255,61,154], // subject B, equally bright, and nothing else
  gold:[255,201,60],    // the one hero or champion the story follows
  amber:[255,122,26],   // what is held back: the locked test, the vault, the reveal
...
// neutral inks here, so no piece of furniture can ever print in a subject's colour by accident.
...
/* =====================================================================
   CHARACTERS — flies with wings, installed on each pen. For a project that is not about flies,
   swap these three for one other non-human shape and keep it the only character in the world.
   ===================================================================== */
...
function installFlies(pen){
  ...
  /* A fly character standing, sitting or hovering at room point (i,j,z). s=1 is about the
     height of a chair back. `ink` says who it is: a subject, the hero, or 'slate' for staff. */
  pen.fly=(i,j,z=0,o={})=>{
  ...
  /* An eliminated fly: on its back, legs up, wings flat. For a pile of the eliminated. */
